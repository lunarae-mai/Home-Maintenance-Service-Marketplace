      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        <div className="mb-10">
          ...
          <p className="mt-4 max-w-2xl text-muted-foreground text-lg">
            Select a specialized service below to view elite providers offering their expertise in this domain.
          </p>
        </div>
        ...
        {subServices.length === 0 ? (
          <div className="rounded-2xl border border-border bg-surface p-12 text-center">
            ...
            <p className="mt-2 text-muted-foreground">This category currently has no specialized services available.</p>
          </div>
        ) : (
          <div className="space-y-12">
            {subServices.map((service) => {
              ...
              return (
                <div key={service.id} className="rounded-3xl border border-border bg-surface p-8 shadow-sm">
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                    <div>
                      <h2 className="text-2xl font-bold text-foreground">{service.name}</h2>
                      <p className="mt-2 text-muted-foreground">{service.description || "Professional service delivered by certified experts."}</p>
                    </div>
                    <div className="flex gap-4 shrink-0">
                      <div className="flex flex-col items-end rounded-xl bg-primary/10 px-4 py-3 text-primary">
                        <span className="text-xs font-semibold uppercase tracking-wider">{basePriceLabel}</span>
                        <div className="flex items-center gap-1 mt-1">
                          <DollarSign className="h-4 w-4" />
                          <span className="text-xl font-bold">{service.basePrice || service.averageCost || "Varies"}</span>
                        </div>
                      </div>
                      <div className="flex flex-col items-end rounded-xl border border-border bg-background px-4 py-3 text-foreground">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className="text-lg font-semibold">{service.duration || "60"} min</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {providers.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      {providers.map((provider) => (
                        <div key={provider.providerId} className="group flex flex-col sm:flex-row justify-between gap-4 rounded-2xl border border-border bg-background p-5 transition hover:border-primary/40 hover:shadow-md">
                            ...
                            <div>
                              ...
                              <div className="mt-1.5 flex items-center gap-1.5 text-sm">
                                <div className="flex items-center text-cyan-500">
                                  ...
                                  <span className="ml-1 font-medium">{provider.avgRating?.toFixed(1) || "New"}</span>
                                </div>
                                <span className="text-muted-foreground">•</span>
                                <span className="text-muted-foreground">{provider.experience} yrs exp.</span>
                              </div>
